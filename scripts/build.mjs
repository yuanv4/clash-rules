...
const INPUT_FORMATS = new Set(["raw-list", "clash-yaml", "domain-text"]);
...
export const normalizeConfiguration = (config) => {
  ...
  for (const provider of config.providers) {
    ...
    if (!new Set(["classical", "domain"]).has(behavior)) {
      throw new Error(`Provider ${provider.name} must use behavior classical or domain`);
    }
    ...
    const incompatibleInput = behavior === "domain"
      ? normalizedInputs.some((input) => input.inputFormat !== "domain-text")
      : normalizedInputs.some((input) => input.inputFormat === "domain-text");
    if (incompatibleInput) {
      throw new Error(
        `Provider ${provider.name} behavior ${behavior} requires ${behavior === "domain" ? "domain-text" : "classical inputs"}`
      );
    }
  }
...
const parseDomainTextRules = (text, url) => {
  const entries = [];
  const domainPattern = /^(?:\+\.)?(?=.{1,253}$)(?:[A-Za-z0-9](?:[A-Za-z0-9_-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9_-]{0,61}[A-Za-z0-9])?))*$/u;
  ...
  for (let lineNumber = 0; lineNumber < physicalLines.length; lineNumber += 1) {
    ...
    if (!domainPattern.test(line)) {
      throw new Error(`Invalid domain text rule at ${url}:${lineNumber + 1}: invalid domain ${JSON.stringify(line)}`);
    }
  ...
  for (const { line } of entries) {
    if (!seen.has(line)) {
      seen.add(line);
      rules.push({ type: "DOMAIN-TEXT", value: line, options: [] });
    }
  }
  if (rules.length === 0) {
    throw new Error(`Rule source has no usable rules: ${url}`);
...
export const parseRules = (text, url, inputFormat = "raw-list") => {
  ...
  if (inputFormat === "domain-text") {
    return parseDomainTextRules(text, url);
  }
...
export const renderDomainText = (provider, rules) => {
  ...
  const entries = rules.map((rule) => {
    if (rule.type !== "DOMAIN-TEXT") {
      throw new Error(`Expected domain text rule for ${provider.name}`);
    }
    assertSafeRuleLine(rule.value, provider.name, "rendered rule");
    return rule.value;
  });
  return ["# Generated by scripts/build.mjs; do not edit.", ...provenance, ...entries, ""].join("\n");
};
...
const build = async (outputDir) => {
  ...
  try {
    ...
    for (const provider of providers) {
      ...
      const output = provider.behavior === "domain"
        ? renderDomainText(provider, rules)
        : renderYaml(provider, rules);
      const extension = provider.behavior === "domain" ? "txt" : "yaml";
